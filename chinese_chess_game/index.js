...
function drawChessboard(start_top,end_top){
	for(var i=0;i<5;i++){
		for(var j=0;j<9;j++){
			...
			pen.lineTo(540,start_top+60*i);
			pen.moveTo(60+j*60,start_top);
			pen.lineTo(60+j*60,end_top);
			pen.stroke();
		}
	}
}
...
//初始化棋盘棋子排列
var chessPoses=document.querySelectorAll(".chessPosition");
function initialPieces(){
	...
	//给红黑双方棋子内加上文字，并加上name属性，格式为：red-車、black-車
	...
	for(var i=0;i<2;i++){
		...
		for(var j=0;j<16;j++){
			switch(j){
				...
				case Math.abs(fixNum-8): txt="車"; break;
				case Math.abs(fixNum-1):
				case Math.abs(fixNum-7): txt="馬"; break;
				case Math.abs(fixNum-2): 
				case Math.abs(fixNum-6): if(i==0){txt="相"}else{txt="象"}; break;
				case Math.abs(fixNum-3): 
				case Math.abs(fixNum-5): if(i==0){txt="仕"}else{txt="士"}; break;
				case Math.abs(fixNum-4): if(i==0){txt="帅"}else{txt="将"}; break;
				case Math.abs(fixNum-9): 
				case Math.abs(fixNum-10): txt="炮"; break;
				case Math.abs(fixNum-11):
				case Math.abs(fixNum-12):
				case Math.abs(fixNum-13):
				case Math.abs(fixNum-14):
				case Math.abs(fixNum-15): if(i==0){txt="兵"}else{txt="卒"}; break;
			}
			if(i==0){
...
var jiangNum=85,shuaiNum=4;    //将帅所在坐标的数字
var isFirstEnter=true;
var m=0,s=0;    //计时板的分钟和秒
var starTime;   //计时器
//开始游戏     程序入口
	...
	for(var i=0;i<chessPoses.length;i++){
		chessPoses[i].onclick=function(){
			...
			if($("."+color+"ClickChess") && this.classList.contains(color)==false && canDrop){
				if(takePiece.innerText=="将"){jiangNum=parseInt(this.id.split("-")[1]);}
				else if(takePiece.innerText=="帅"){shuaiNum=parseInt(this.id.split("-")[1]);}
				if(this.innerText=="将"){
					$("#alert-cover").style.display="block";
					$("#window-btn").innerHTML="再来一局";
					clearInterval(startTime);
...
function judgeDrop(){
	var temp=0,idNum=0;
	var takeNum=parseInt(takePiece.id.split("-")[1]);
	var canPos=[];   //把棋手可能走的所有位置都装在这个数组里面
	var fixNum;       //区别红黑双方士、将的数字，红方时，fixNum=0，黑方时，fixNum=63
	if(color=="red"){fixNum=0;}
	else if(color=="black"){fixNum=63;}
	...
	function car(){
		for(var i=0;i<4;i++){     //从上下左右四个方向看看有没有棋子挡路
			if((i==1 && takeNum%9==8) || (i==3 && takeNum%9==0)){     //当棋子在最右侧或者最左侧的纵轴上时，不允许往右和往左寻找可能的位置
				...
				switch(i){
					case 0: temp=9; break;    //上
					case 1: temp=-1; break;   //右
					case 2: temp=-9; break;    //下
					case 3: temp=1; break;    //左
				}
				idNum=takeNum-temp;
				if(idNum>=0 && idNum<=89){
	...
	function horse(){
		...
		for(var i=0;i<8;i++){     //从右竖日上、右横日上、右横日下、右竖日下……八个方向看看有没有棋子挡路
			switch(i){
				case 0: temp=17,poorNum=8; break;    //右竖日上
				case 1: temp=7,poorNum=8; break;   //右横日上
				case 2: temp=-11,poorNum=-10; break;   //右横日下
				case 3: temp=-19,poorNum=-10; break;    //右竖日下
				case 4: temp=-17,poorNum=-8; break;    //左竖日下
				case 5: temp=-7,poorNum=-8; break;    //左横日下
				case 6: temp=11,poorNum=10; break;    //左横日上
				case 7: temp=19,poorNum=10; break;    //左竖日上
			}
			idNum=takeNum-temp;
			if(idNum>=0 && idNum<=89){
	...
	function xiang(){
		for(var i=0;i<4;i++){
			switch(i){
				case 0: temp=16; break;   //右上
				case 1: temp=-20; break;  //右下
				case 2: temp=-16; break;  //左下
				case 3: temp=20; break;   //左上
			}
			idNum=takeNum-temp;
			if(idNum>=0 && idNum<=89){
	...
	function shi(){
		for(var i=0;i<4;i++){
			switch(i){
				case 0: temp=8; break;   //右上
				case 1: temp=-10; break;  //右下
				case 2: temp=-8; break;  //左下
				case 3: temp=10; break;   //左上
			}
			idNum=takeNum-temp;
	...
	function jiang(){
		...
		if((jiangNum-shuaiNum)%9==0){   //说明将帅处在同一纵轴上
			...
			if(!hasPiece){
				if(color=="red"){canPos.push(jiangNum);}
				else if(color=="black"){canPos.push(shuaiNum);}
			}
		}
		console.log("可落子的位置："+canPos);
	}
	...
	function bing(){
		...
		for(var i=0;i<4;i++){
			switch(i){
				case 0: if(color=="black"){temp=9}; break;     //上
				case 1: if((color=="red" && takeNum>44)||(color=="black" && takeNum<45)){temp=-1;}  break;    //右
				case 2: if(color=="red"){temp=-9}; break;    //下
				case 3: if((color=="red" && takeNum>44)||(color=="black" && takeNum<45)){temp=1;} break;     //左
			}
			idNum=takeNum-temp;
			if(idNum>=0 && idNum<=89){
				if(!$("#P-"+idNum).classList.contains(color)){
		...
		for(var i=0; i<tempArr.length;i++) {
	        ...
	        if(tempArr.indexOf(tempArr[i])==i){canPos.push(tempArr[i])}
	    }
		console.log("可落子的位置："+canPos);
	}
	putChess(canPos);
